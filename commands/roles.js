...
module.exports.run = async (client, message, args) => {
    ...
    if (roleName) {
        ...
        if (!roleInfo) return message.channel.send("Role does not exist.");
    ...
    if (user && !roleName) {
        ...
        con.query(`SELECT IDRole FROM rollen WHERE IDUser = '${user.id}'`, (err, rows) => {
            ...
            if (rows.length > 0) {
                ...
                var listRoles = "**Role (s) of this person: ** \n\n";
        ...
        con.query(`SELECT * FROM rollen WHERE IDUser = '${user.id}' AND IDRole = '${roleID}'`, (err, rows) => {
            ...
            if (rows.length > 0) {
                ...
                return message.channel.send("Role is Added");
        ...
        con.query(`DELETE FROM rollen WHERE IDUser = '${user.id}' AND IDRole = '${roleID}'`, (err, rows) => {
            ...
            if (rows.affectedRows == 1) {
                ...
                return message.channel.send(`Removed from role ${roleName}.`);
        ...
        message.channel.send("Use the command like this: !role Username roleName verwijderen");
    }
